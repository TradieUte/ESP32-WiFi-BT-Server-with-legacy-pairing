...
#if (!defined(CONFIG_BT_SSP_ENABLED) && defined(CONFIG_BT_CLASSIC_ENABLED))
volatile esp_bt_pin_code_t legacyPin;
volatile int legacyPin_len;
#endif
...
#if (!defined(CONFIG_BT_SSP_ENABLED) && defined(CONFIG_BT_CLASSIC_ENABLED)) // Used during Classic legacy pairing
static void esp_bt_gap_slave_cb(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
  switch (event)
  {
  case ESP_BT_GAP_PIN_REQ_EVT:
    ...
    {
      log_i("Input pin code - length 16");
    ...
    {
      log_i("Input pin code - length 4 P:%s L:%d",legacyPin,legacyPin_len);
      esp_bt_gap_pin_reply(param->pin_req.bda, true, legacyPin_len, (uint8_t *)legacyPin);
    }
    break;
...
#if (!defined(CONFIG_BT_SSP_ENABLED) && defined(CONFIG_BT_CLASSIC_ENABLED))
  if (esp_bt_gap_register_callback(esp_bt_gap_slave_cb) != ESP_OK) {
#else
...
#if (!defined(CONFIG_BT_SSP_ENABLED) && defined(CONFIG_BT_CLASSIC_ENABLED))
    _pin_len = 0; // forces legacy pairing
    legacyPin_len = strlen(pin);
    for (int i =0; i < legacyPin_len; i++)
    {
      legacyPin[i] = (uint8_t)pin[i];
    }
    legacyPin[4] = '\0';
    log_i("PIN %s LEGACY_PIN %s",pin,legacyPin);
#else
        _pin_len = strlen(pin);
#endif
